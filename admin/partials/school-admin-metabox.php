...
function exercise_meta_box_markup($object)
{
    wp_enqueue_editor();
    wp_nonce_field(basename(__FILE__), "meta-box-nonce"); ?>

    <div id="exercise-btn-table" style="width:100%">
        <ul>
            <li><button id="exercise-box-btn" type="button">Add exercise</button></li>
            <li><select id="type">
    ...
    <div id="div-exercise-wraper"></div>
...
{
    add_meta_box("demo-meta-box", "Exercises in the classes", "exercise_meta_box_markup", "classes", "normal", "high", null);
}
add_action("add_meta_boxes", "add_exercise_meta_box");

function save_exercisemeta_box($post_id, $post, $update)
{
    if (!isset($_POST["meta-box-nonce"]) || !wp_verify_nonce($_POST["meta-box-nonce"], basename(__FILE__)))
        return $post_id;
    if(!current_user_can("edit_post", $post_id))
...
add_action("save_post", "save_exercise_meta_box", 10, 3);

add_action( 'wp_ajax_add_exercise', 'add_exercise' );
function add_exercise() {

    $type = $_POST['whatever'];

    if ( $type == "Text" ) {
        $html = '<div id="exercise-div" data-exercise="">';
        $html .= '<textarea id="w3mission" rows="4" cols="50"></textarea>';
        $html .= '</div>';
    }
    ...
    if ( $type == "Checkbox" ) {
        $html = '<div id="exercise-div" data-exercise="">';
            ...
            $html .= '<label for="vehicle1"> I have a bike</label><br>';
            $html .= '<input type="checkbox" id="vehicle2" name="vehicle2" value="Car">';
            $html .= '<label for="vehicle2"> I have a car</label><br>';
            $html .= '<input type="checkbox" id="vehicle3" name="vehicle3" value="Boat">';
            $html .= '<label for="vehicle3"> I have a boat</label><br><br>';
            $html .= '</form>';
        $html .= '</div>';
    }
    ...
    if ( $type == "Radio" ) {
        $html = '<div id="exercise-div" data-exercise="">';
            ...
            $html .= '<input type="radio" id="other" name="gender" value="other">';
...
add_action( 'admin_footer', 'add_exercise_javascript' ); // Write our JS below here
function add_exercise_javascript() { ?>
	<script type="text/javascript" >
    function add_exercise_data($) {
            var active_editors = [];

            $("#exercise-box-btn").on( "click", function(event) {
            event.preventDefault();

                var data = {
                    'action': 'add_exercise',
                    'postid': $("#chart-data-table").attr('data-chartid'),
                    'whatever': $("#type option:selected").text(),
                ...
                jQuery.post(ajaxurl, data, function(response) {
                    ...
                    $("#div-exercise-wraper").append(response);
            });
